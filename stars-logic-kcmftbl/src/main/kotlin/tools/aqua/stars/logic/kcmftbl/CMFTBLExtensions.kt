...
fun <E1 : E, E : EntityType<E, T, S>, T : TickDataType<E, T, S>, S : SegmentType<E, T, S>> previous(
    ...
    previous(
        ...
        eval = { td ->
          ...
          if (entity::class.isInstance(previousEntity)) eval(entity::class.cast(previousEntity))
          else false
        })
...
fun <E1 : E, E : EntityType<E, T, S>, T : TickDataType<E, T, S>, S : SegmentType<E, T, S>> next(
    ...
    next(
        ...
        eval = { td ->
          ...
          if (entity::class.isInstance(nextEntity)) eval(entity::class.cast(nextEntity)) else false
        })
...
fun <E1 : E, E : EntityType<E, T, S>, T : TickDataType<E, T, S>, S : SegmentType<E, T, S>> once(
    ...
    interval: Pair<Double, Double> = Pair(0.0, Double.MAX_VALUE),
    eval: (E1) -> Boolean
): Boolean = since(entity, interval, phi1 = { _ -> true }, phi2 = { a -> eval(a) })

fun <E : EntityType<E, T, S>, T : TickDataType<E, T, S>, S : SegmentType<E, T, S>> once(
    tickData: T,
    interval: Pair<Double, Double> = Pair(0.0, Double.MAX_VALUE),
    eval: (T) -> Boolean
): Boolean = since(tickData, interval, phi1 = { _ -> true }, phi2 = { td -> eval(td) })
...
fun <
    ...
    interval: Pair<Double, Double> = Pair(0.0, Double.MAX_VALUE),
    eval: (E1, E2) -> Boolean
): Boolean = !once(entity1, entity2, interval, eval = { a1, a2 -> !eval(a1, a2) })
...
fun <
    ...
    interval: Pair<Double, Double> = Pair(0.0, Double.MAX_VALUE),
    eval: (E1) -> Boolean
): Boolean = !once(entity, interval, eval = { a -> !eval(a) })

fun <E : EntityType<E, T, S>, T : TickDataType<E, T, S>, S : SegmentType<E, T, S>> historically(
    tickData: T,
    interval: Pair<Double, Double> = Pair(0.0, Double.MAX_VALUE),
    eval: (T) -> Boolean
): Boolean = !once(tickData, interval, eval = { td -> !eval(td) })
...
fun <
    ...
    interval: Pair<Double, Double> = Pair(0.0, Double.MAX_VALUE),
    eval: (E1) -> Boolean
): Boolean = until(entity, interval, phi1 = { _ -> true }, phi2 = { a -> eval(a) })

fun <E : EntityType<E, T, S>, T : TickDataType<E, T, S>, S : SegmentType<E, T, S>> eventually(
    tickData: T,
    interval: Pair<Double, Double> = Pair(0.0, Double.MAX_VALUE),
    eval: (T) -> Boolean
): Boolean = until(tickData, interval, phi1 = { _ -> true }, phi2 = { td -> eval(td) })
...
fun <
    ...
    E : EntityType<E, T, S>,
    T : TickDataType<E, T, S>,
    S : SegmentType<E, T, S>> globally(
    entity1: E1,
    entity2: E2,
    interval: Pair<Double, Double> = Pair(0.0, Double.MAX_VALUE),
    eval: (E1, E2) -> Boolean
): Boolean = !eventually(entity1, entity2, interval, eval = { a1, a2 -> !eval(a1, a2) })

fun <E1 : E, E : EntityType<E, T, S>, T : TickDataType<E, T, S>, S : SegmentType<E, T, S>> globally(
    entity: E1,
    interval: Pair<Double, Double> = Pair(0.0, Double.MAX_VALUE),
    eval: (E1) -> Boolean
): Boolean = !eventually(entity, interval, eval = { a -> !eval(a) })

fun <E : EntityType<E, T, S>, T : TickDataType<E, T, S>, S : SegmentType<E, T, S>> globally(
    tickData: T,
    interval: Pair<Double, Double> = Pair(0.0, Double.MAX_VALUE),
    eval: (T) -> Boolean
): Boolean = !eventually(tickData, interval, eval = { td -> !eval(td) })
                ...
                currentTickData.entities
                    ...
                    ?.let { nextEntity1 ->
                      currentTickData.entities
                          ...
                          ?.let { nextEntity2 -> eval(nextEntity1 as E1, nextEntity2 as E2) }
                    }
                    ?: false
          }
          .count { it }
...
fun <
    ...
    minPrevalence(entity1, entity2, 1 - percentage, interval, eval = { a1, a2 -> !eval(a1, a2) })
