...
class TestForall {
  ...
  fun `Test when not all elements match the predicate`() {
    ...
    assertFalse(forAll(list) { it % 2 == 0 })
  }
  ...
  /** Test when first element does not match the predicate. */
  @Test
  fun `Test when first element does not match the predicate`() {
    val list = listOf(1, 4, 6, 8)
    assertFalse(forAll(list) { it % 2 == 0 })
  }

  /** Test when last element does not match the predicate. */
  @Test
  fun `Test when last element does not match the predicate`() {
