...
import tools.aqua.stars.logic.kcmftbl.future.release

/** This class tests the CMFTBL operator [release]. */
class TestRelease {
  ...
  fun `Test when phi1 is true at current time but phi2 is not, while no interval is given`() {
    val phi1 = listOf(1, 0, 0)
    val phi2 = listOf(0, 0, 0)

    assertFalse {
      release(createTicks(phi1, phi2)[0], null, phi1 = { it.phi1 }, phi2 = { it.phi2 })
    }
  }
  ...
  fun `Test when phi1 is true at first two ticks but phi2 is only on first, while no interval is given`() {
    ...
    assertTrue { release(createTicks(phi1, phi2)[0], null, phi1 = { it.phi1 }, phi2 = { it.phi2 }) }
  }
