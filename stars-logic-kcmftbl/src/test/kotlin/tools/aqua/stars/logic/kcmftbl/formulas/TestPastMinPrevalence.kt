...
import tools.aqua.stars.logic.kcmftbl.past.pastMinPrevalence

/** This class tests the CMFTBL operator [pastMinPrevalence]. */
class TestPastMinPrevalence {
  ...
  fun `Test when phi is true and percentage is 80, while no interval is given`() {
    ...
    assertTrue { pastMinPrevalence(createTicks(phi)[9], percentage, null, phi = { it.phi1 }) }
  }
  ...
  fun `Test when phi is true for 7 of 10 ticks and percentage is 80, while no interval is given`() {
    val phi = listOf(1, 1, 1, 0, 1, 0, 1, 0, 1, 1)
    val percentage = 0.8

    assertFalse { pastMinPrevalence(createTicks(phi)[9], percentage, null, phi = { it.phi1 }) }
  }
  ...
  fun `Test when phi is false but percentage is 0, while no interval is given`() {
    ...
    assertTrue { pastMinPrevalence(createTicks(phi)[2], percentage, null, phi = { it.phi1 }) }
  }
  ...
  fun `Test when phi is true for 8 of 10 ticks and percentage is 80, and 3 of 5 in interval are true`() {
    val phi = listOf(1, 1, 1, 1, 1, 1, 0, 0, 1, 1)
    val percentage = 0.8
    val interval = 0 to 4
  ...
  fun `Test when phi is true and percentage is 100, but no tick is in the interval`() {
    ...
    assertTrue {
      pastMinPrevalence(ticks[2], percentage, createInterval(interval), phi = { it.phi1 })
    }
  }
  ...
  fun `Test percentage less than 0`() {
    ...
    assertFailsWith<IllegalArgumentException> {
      pastMinPrevalence(createTicks(phi)[2], percentage, null, phi = { it.phi1 })
    }
  }
