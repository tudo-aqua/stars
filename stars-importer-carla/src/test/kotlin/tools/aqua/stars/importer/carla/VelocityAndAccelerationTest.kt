...
class VelocityAndAccelerationTest {
  ...
  fun testVelocityCalculation() {
    ...
    val tickData1 = TickData(currentTick = TickDataUnitSeconds(0.0), entities = listOf(vehicle1))
    val tickData2 = TickData(currentTick = TickDataUnitSeconds(1.0), entities = listOf(vehicle2))
    val tickData3 = TickData(currentTick = TickDataUnitSeconds(2.0), entities = listOf(vehicle3))

    // Vehicle 0 has no previous state. The velocity and acceleration values should be set to 0.0
    updateActorVelocityAndAcceleration(vehicle = vehicle1, previousActor = null)
    ...
    updateActorVelocityAndAcceleration(vehicle = vehicle2, previousActor = vehicle1)
    ...
    updateActorVelocityAndAcceleration(vehicle = vehicle3, previousActor = vehicle2)
  ...
  fun testEmptyPreviousVehicle() {
    ...
    updateActorVelocityAndAcceleration(vehicle = vehicle, previousActor = null)
  ...
  fun testWrongActorType() {
    ...
    assertFailsWith<IllegalStateException> {
      updateActorVelocityAndAcceleration(vehicle = vehicle1, previousActor = actor0)
    }
  }
}
