...
class JSONTrafficLightTest {
    ...
    fun testStaticTrafficLightConversion() {
        ...
        staticJsonTrafficLight.stopLocations.forEach { jsonLocation ->
            assertNotNull(staticTrafficLight.stopLocations.firstOrNull { location ->
                jsonLocation.x == location.x && jsonLocation.y == location.y && jsonLocation.z == location.z
            })
        }
    }
    ...
    fun testJsonTrafficLightToTrafficLightConversion() {
        ...
        assertEquals(
            trafficLight.state, tickData1.blocks[0].roads[0].lanes[0].trafficLights[0].getStateInTick(tickData1)
        )

        assertNotNull(tickData2.blocks[0].roads[0].lanes[0].trafficLights[0].getStateInTick(tickData2))
        assertEquals(
            trafficLight2.state, tickData2.blocks[0].roads[0].lanes[0].trafficLights[0].getStateInTick(tickData2)
        )
    }
