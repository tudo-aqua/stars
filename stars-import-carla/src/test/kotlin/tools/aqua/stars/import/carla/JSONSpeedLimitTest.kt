...
class JSONSpeedLimitTest {
    ...
    fun testNonSpeedLimitLandmarks() {
        val invalidLandmarks: List<JsonLandmark> =
            JsonLandmarkType.values().filter { it != JsonLandmarkType.MaximumSpeed }.map {
    ...
    fun testSingleSpeedLimitBoundariesStartingAtEndOfLane() {
        ...
        assertFailsWith<IllegalStateException> {
            getSpeedLimitsFromLandmarks(
                speedLimitLane,
                listOf(speedLimitLandmark)
            )
        }
    }
    ...
    fun testApplicableSpeedLimitsForVehicle() {
        val speedLimit1 = SpeedLimit(
            speedLimit = 30.0,
            fromDistanceFromStart = 10.0,
            toDistanceFromStart = 30.0
        )
        val speedLimit2 = SpeedLimit(
            speedLimit = 50.0,
            fromDistanceFromStart = 30.0,
            toDistanceFromStart = 50.0
        )
        val speedLimit3 = SpeedLimit(
            speedLimit = 80.0,
            fromDistanceFromStart = 50.0,
            toDistanceFromStart = 80.0
        )
