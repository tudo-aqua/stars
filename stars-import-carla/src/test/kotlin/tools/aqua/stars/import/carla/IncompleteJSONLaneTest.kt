...
class IncompleteJSONLaneTest {
    ...
    fun setupData() {
        ...
        incompleteRoad = Road(
            lanes = listOf(),
            id = 1, block = incompleteBlock, isJunction = false
        )
        incompleteJsonLane = JsonLane(
            laneId = 1,
            roadId = 1,
            s = 0.0,
    ...
    fun checkSettingOfIntersectingLanes() {
        incompleteJsonLane.intersectingLanes.forEach { jsonContactInfo ->
            assertNull(incompleteLane.intersectingLanes.firstOrNull { contactInfo ->
    ...
    fun checkMissingLanes() {
        incompleteJsonLane.contactAreas.forEach { jsonContactArea ->
            assertNull(incompleteLane.contactAreas.firstOrNull { contactArea ->
