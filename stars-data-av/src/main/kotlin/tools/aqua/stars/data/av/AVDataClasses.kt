...
data class Segment(
    ...
    override val egoVehicleId: Int
        get() {
            ...
            if (tickData.any { it.egoVehicle.id != firstEgo.id }) error("inconsistent ego ids in segment $this")
            return firstEgo.id
        }
...
data class TickData(
    ...
    val pedestrians: List<Pedestrian> get() = actors.filterIsInstance<Pedestrian>()
...
data class Lane(
    ...
    /**
     * lanes that this lane must yield to according to the "right before left" rule,
     * i.e., traffic signs or lights are not taken into account.
     *
     ...
     */
    fun speedAt(vPos: Double): Double = speedLimits.firstOrNull {
        it.fromDistanceFromStart <= vPos && vPos < it.toDistanceFromStart
    }?.speedLimit ?: 30.0
    ...
    /**
     * retrieve the position (relative to this lane's start) where [otherLane]
     * is crossed. if the lanes do not cross, return -1.0
     */
    fun contactPointPos(otherLane: Lane): Double? {
        ...
        val positions = contactAreas.mapNotNull {
            ...
            if (
                this == it.lane1 && otherLane == it.lane2
            ) {
                it.lane1StartPos
            }
            // I am lane 2, check if (and where) lane 1 contacts
            else if (
                this == it.lane2 && otherLane == it.lane1
            ) {
                it.lane2StartPos
            } else {
...
data class Pedestrian(
    ...
    override fun clone(newTickData: TickData): Actor = Pedestrian(id, newTickData, positionOnLane, lane)
...
data class Vehicle(
    ...
    val applicableSpeedLimit
        get() = this.lane.speedLimits.firstOrNull { speedLimit ->
    ...
    override fun clone(newTickData: TickData): Actor = Vehicle(
        id,
        newTickData,
        positionOnLane,
        lane,
