...
)
{
     ...
     *
     */
    fun holds(ctx: PredicateContext<E,T,S>, tickId: Double = ctx.segment.firstTickId, actorId: Int = ctx.egoID) = ctx.holds(this, tickId, actorId)
    fun holds(ctx: PredicateContext<E,T,S>, entity: E) = holds(ctx, entity.tickData.currentTick, entity.id)
    ...
    val klass: Pair<KClass<E1>, KClass<E2>>)
{
     ...
     *
     */
    fun holds(ctx: PredicateContext<E,T,S>, tickId: Double = ctx.segment.firstTickId, actor1Id: Int = ctx.egoID, actor2Id: Int) = ctx.holds(this, tickId, actor1Id, actor2Id)
    fun holds(ctx: PredicateContext<E,T,S>, actor1: E1, actor2: E2) = holds(ctx,
        actor1.tickData.currentTick.apply { if (this != actor2.tickData.currentTick) error("ticks don't match") },
        actor1.id,
        actor2.id)
}
