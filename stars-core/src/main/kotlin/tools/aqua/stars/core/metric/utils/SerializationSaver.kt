...
fun saveAsJSONFile(serializableResult: SerializableResult) {
  ...
  File("$resultFolder/${serializableResult.identifier?:"result"}.csv").apply {
    createNewFile()
...
private fun getAndCreateJSONFolder(source: String): String =
    "${ApplicationConstantsHolder.SERIALIZED_RESULTS_FOLDER}/${ApplicationConstantsHolder.applicationStartTimeString}/$source"
        .also { File(it).mkdirs() }
