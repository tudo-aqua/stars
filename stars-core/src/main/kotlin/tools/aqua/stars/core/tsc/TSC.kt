...
abstract class TSCNode<E: EntityType, T: TickDataType<E>,S: SegmentType<E, T>>(
    ...
    override fun hashCode(): Int = valueFunction.hashCode() + monitorFunction.hashCode() +
            projectionIDWrappers.hashCode() + edges.sumOf { it.hashCode() }

    /**
     * evaluates this TSC in the given context
     */
    fun evaluate(ctx: PredicateContext<E,T,S>, depth: Int = 0): TSCInstanceNode<E,T,S> {
        val instanceNode = TSCInstanceNode(
            this.valueFunction(ctx),
            this.monitorFunction(ctx),
            this
        )
        this.edges.forEach { tscEdge ->
            if (tscEdge.condition(ctx))
                instanceNode.edges += TSCInstanceEdge(
                    tscEdge.label,
    ...
    fun buildProjections(): Map<Any, TSCNode<E,T,S>> {
        ...
        projectionIDWrappers.keys.forEach { id ->
            buildProjection(id)?.let { returnMap += id to it }
        }
        return returnMap
    }
        ...
        when (projectionIDWrappers[projectionID]) {
            ...
            false -> {
                val outgoingEdges = edges
                    ...
                    .mapNotNull {
                        when (it.first) {
                            is TSCAlwaysEdge -> it.second?.let { _ -> TSCAlwaysEdge(it.first.label, it.second!!) }
                            else -> it.second?.let { _ -> TSCEdge(it.first.label, it.first.condition, it.second!!) }
                        }
                    }.toTypedArray()
                ...
                return when (this) {
                    ...
                    is TSCBoundedNode -> TSCBoundedNode(
                        this.valueFunction,
    ...
    /**
     * same output as toString, but with labels on the edges
     * provided by [labels].
     */
    ...
    private fun toStringWithEdgeLabels(depth: Int, labels: Map<TSCEdge<E,T,S>, Any>): String {
        ...
        this.edges.forEach { instanceEdge ->
            builder.append("  ".repeat(depth))
            when (instanceEdge) {
                is TSCAlwaysEdge -> builder.append("-[${labels[instanceEdge] ?: 0}]-> ${instanceEdge.label} ")
...
class TSCBoundedNode<E: EntityType, T: TickDataType<E>,S: SegmentType<E, T>>(
    ...
    override fun hashCode(): Int = valueFunction.hashCode() + monitorFunction.hashCode() +
            projectionIDWrappers.hashCode() + bounds.hashCode() + edges.sumOf { it.hashCode() }
    ...
    override fun generateAllInstances(): List<TSCInstanceNode<E,T,S>> {
        ...
        // build all subsets of allSuccessorsList and filter to subsets.size in bounds.first..bounds.second
        val boundedSuccessors = allSuccessorsList.powerlist().filter { subset ->
            subset.size in bounds.first..bounds.second
        }.toList()

        boundedSuccessors.forEach { subset ->
            when (subset.size) {
                0 -> returnList += TSCInstanceNode(Unit, true, this)
                1 -> subset.first().forEach { successors ->
                    val generatedNode = TSCInstanceNode(Unit, true, this)
                    successors.forEach { successor ->
                        generatedNode.edges += successor
                    }
                    returnList += generatedNode
                }

                else -> crossProduct(subset).forEach { successors ->
                    val generatedNode = TSCInstanceNode(Unit, true, this)
                    successors.forEach { successor ->
                        generatedNode.edges += successor
                    }
...
/**
 *  Build all possible combinations of the lists in the input list.
 *  Example instances kept until better documentation will be written.
 *
 *    val input = listOf(
...
fun <T> crossProduct(input: List<List<List<T>>>): List<List<T>> {
    require(input.size >= 2) { "Input list for cross-product building must at least contain two elements. received $input" }
    ...
    return if (input.size == 2)
        ...
        crossProduct(listOf(nextLevelList) + input.subList(2, input.size))
...
class TSCInstanceNode<E: EntityType, T: TickDataType<E>,S: SegmentType<E, T>>(
    ...
    /**
     * Validates own (and recursively all children's)  successor constraints imposed
     * by the [TSCNode<E,T,S>] types the instances were built from (e.g. exactly one for [TSCXorNode]
     * or correct range for [TSCBoundedNode])
     *
     * @param label the label used to build the human-readable string; uses [TSCEdge.label] in recursive call
     *
     * @return non-validating nodes; first element of pair is the node that failed to validate;
     * second element is a human-readable explanation for the failure
     */
    fun validate(label: String = "RootNode"): List<Pair<TSCInstanceNode<E,T,S>, String>> {
        ...
        when (this.tscNode) {
            ...
            is TSCBoundedNode -> if (edges.size !in tscNode.bounds.first..tscNode.bounds.second)
                returnList += this to "[$label] TSCBoundedNode successor count must be within [${tscNode.bounds.first},${tscNode.bounds.second}], but ${edges.size} successor(s) found."
        }
    ...
    /**
     * Validates own (and recursively all children's) results of the [TSCNode<E,T,S>.monitorFunction] results
     * and collects incoming edge labels for results != true.
     *
     * @param label the label added to the return list if [monitorResult] == `false`; uses [TSCEdge.label] in recursive call
     *
