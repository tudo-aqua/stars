 ...
 * always contains one edge and the node that belongs to that edge (edge.destination = node)
...
class TSCBuilder<E: EntityType<E,T,S>, T: TickDataType<E,T,S>,S: SegmentType<E,T,S>>(var label: String = "") {
    ...
    fun buildBounded(): TSCEdge<E,T,S> {
        val node = TSCBoundedNode(valueFunction, monitorFunction, projectionIDs, bounds, *edges.toTypedArray())
        return condition?.let { cond -> TSCEdge(label, cond, node) } ?: TSCAlwaysEdge(label, node)
    }


    /**
     * adds the given edge to "edges" - this will become the edges of the node that will be created off of this object
     * @param edge
     */
