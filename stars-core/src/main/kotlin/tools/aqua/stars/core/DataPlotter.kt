...
fun plotUniqueTSCInstances(nameList: List<String>, dataLists: List<List<Int>>, plotFolder: String, dataFolder: String) {
    ...
    val data = mapOf(
        ...
        "projection" to List(dataLists.size) { index -> List(indices.size) { nameList[index] } }.flatten()
    )

    var plot = letsPlot(data) { x = "segment index"; y = "unique instances"; color = "projection"; fill = "projection" }
    plot += ggsize(500, 250)

    // Default histogram (stacked)
    //p1 += geomLine(stat = Stat.identity)
    plot += geomLine(stat = Stat.identity, position = positionDodge(POSITION_DODGE)) +
...
fun plotUniqueTSCInstancePercentages(
    ...
    var plot =
        letsPlot(data) {
            x = "segment index"; y = "unique instances (in %)"; color = "projection"; fill =
            "projection"
        }
    plot += ggsize(500, 250)
...
fun plotTSCInstanceOccurrencesForProjection(
    ...
    var plot = letsPlot(data) { x = "instance index"; y = "instance count"; color = "projection"; fill = "projection" }
    plot += ggsize(500, 250)
...
fun writeCSVFilesFromDataList(dataList: List<Int>, plotName: String, projectionName: String, dataFolder: String) {
    val file = File("$dataFolder/${plotName}-${projectionName.replace("/", "-")}.csv")
    file.createNewFile()

    val fileOnlyEvery200th = File("$dataFolder/${plotName}-${projectionName.replace("/", "-")}-200th.csv")
...
fun writeCSVFilesFromDataLists(
    dataList: List<List<Int>>, nameList: List<String>, plotName: String, dataFolder:
    String
) {
